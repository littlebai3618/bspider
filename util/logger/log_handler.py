...
class RabbitMQLogHandler(logging.Handler):
    ...
    def __init__(self,mq_handler, level='INFO'):
        ...
        # 创建交换机
        self.mq_handler.declare_exchange(frame_settings.LOGGER_EXCHANGE_NAME)

    def emit(self, record):
        self.mq_handler.send_message(
            exchange=frame_settings.LOGGER_EXCHANGE_NAME,
            routing_key=frame_settings.LOGGER_EXCHANGE_NAME,
...
class LoggerPool(object):
    ...
    def __init__(self):
        self.__pool = {}
        if frame_settings.GLOBAL_PATTERN == 'produce':
    ...
    def get_logger(self, key, level='DEBUG', **kwargs) -> logging.Logger:
        key = '{}:{}:{}'.format(key, kwargs.get('module', ''), kwargs.get('project', ''))
        if key in self.__pool:
            return self.__pool[key]
        log_handler = self.__handle_func(logging.getLogger(key), level, **kwargs)

...
from util.exceptions.exceptions import DownloaderError
...
class AsyncDownloader(object):
    ...
    def __init__(self, job_name: str, config: dict, sign: str):
        """传入下载器的配置文件"""
        self.sign = sign
        self.log = LoggerPool().get_logger(key=job_name, level='INFO', module='downloader', project=job_name)
        ...
        if 'RETRY_TIMES' in settings and isinstance(settings['RETRY_TIMES'], int):
            ...
            self.log.debug(f'{job_name} init retry time from settings: {self.retry_times}')
        else:
            self.retry_times = 3
            self.log.debug(f'{job_name} init default retry time: {self.retry_times}')
        self.mws = []
        for cls_name, code in mws:
            mod = ModuleImporter.import_module(cls_name, code, job_name)
            if mod and hasattr(mod, cls_name):
                try:
                    ...
                    self.log.info(f'success load: <{job_name}:{cls_name}>!')
                except Exception as e:
                    raise DownloaderError(f'<{job_name}:{cls_name}> middleware init failed: {e}')

            else:
                msg = f'<{job_name}:{cls_name}> middleware init failed: middleware is invalid!'
        ...
        if 'ACCEPT_RESPONSE_CODE' in settings and isinstance(settings['RETRY_TIMES'], list):
            ...
            self.log.debug(f'{job_name} init accept response code from settings: {self.accept_response_code}')
        else:
            self.accept_response_code = set()
            self.log.debug(f'{job_name} init default accept response code: {self.accept_response_code}')
    ...
    async def download(self, request: Request):
        ...
        for retry_index in range(self.retry_times):
            ...
            try:
                ...
                is_req = True
                for mw in self.mws:
                    req = await mw.process_request(request)
                ...
                # 执行下载异常中间件
                for mw in self.mws:
                    response = await mw.process_exception(request, e, response)
                    if isinstance(response, Response):
                        continue
                    elif response is None:
                        return None
